...
import co.qingyu.dbtransfer.config.FieldRelation;
import co.qingyu.dbtransfer.config.TransferRelation;
import co.qingyu.dbtransfer.consts.TransferType;
import co.qingyu.dbtransfer.log.TransferLogger;
import co.qingyu.dbtransfer.type.TypeHandler;
import co.qingyu.dbtransfer.util.MateDataUtils;
import co.qingyu.dbtransfer.util.PlaceholderUtil;
...
//    private static final Logger logger = LoggerFactory.getLogger(SimpleDataBaseTransfer.class);
    ...
    public void transfer() throws TransferException {
        ...
        try {
            ...
            TransferLogger.info("---------------------------------> transfer success");
        } catch (Exception e) {
    ...
    private void transferSql(Map<String, SqlTemplate> sqlTemplateMap) throws SQLException, IOException {
        ...
        try {
            for (Map.Entry<String, SqlTemplate> entry : sqlTemplateMap.entrySet()) {
                TransferLogger.info("----------------> 开始解析 [ " + entry.getKey() + " ] <----------------");
                TransferLogger.info("----------------> Execute [ " + entry.getValue().getSourceSelectSql() + " ] <----------------");
                ...
                try {
                    ...
                    TransferLogger.info("----------------> 解析成功 [ " + entry.getKey() + "] <----------------");
                } catch (SQLException e) {
                    TransferLogger.error(e);
                    TransferLogger.info("----------------> 解析失败 [ " + entry.getKey() + "] <----------------");
                }
            }
        } finally {
            writer.close();
    ...
    private void transferDB(Map<String, SqlTemplate> sqlTemplateMap) {
        ...
        try {
            for (Map.Entry<String, SqlTemplate> entry : sqlTemplateMap.entrySet()) {
                TransferLogger.info("begin parsing " + entry.getKey());
                ...
                try {
                    ...
                    TransferLogger.error("------------------------------------> Table [" + entry.getKey() + "] 数据同步失败");
                } finally {
                    this.closeResource(statement, rs);
                }
                TransferLogger.info("end parsing " + entry.getKey());
            }
        } catch (Exception e) {
            TransferLogger.error(e);
        } finally {
    ...
    private void saveDataTo(ResultSet dataRs, SqlTemplate sqlTemplate) {
        ...
        try {
            if (!dataRs.next()) {
                TransferLogger.warn(sqlTemplate.getSourceTableName() + " is an empty table.");
            } else {
                ...
                do {
                    for (int i = 1; i <= placeholders.length; i++) {
                        ...
                        try {
                            ...
                            TransferLogger.error("set parameter failure; prompt: [" + fieldStr + ":" + type + "] match [" + targetSqlField.getName() + ":" + targetSqlField.getType() + "] ");

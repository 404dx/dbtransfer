...
import co.qingyu.dbtransfer.config.DataSourceConfig;
    ...
    public static TransferConfig getTransferConfig(String path) throws TransferException {
        try {
            ...
            dataSources.elements().forEach(o -> {
                ...
                dataSourceConfig.setDbType(elem.element("dbType").getText());
...
/*            List<TransferRelation> relationList = new ArrayList<>();
    ...
    private static boolean isNotBasicDataType(Class clazz) {
        return !int.class.isAssignableFrom(clazz) && !double.class.isAssignableFrom(clazz) && !long.class.isAssignableFrom(clazz)
